 ...
 window.initVoltVoidModel = function initVoltVoidModel(stage, canvas, opts = {}) {
  ...
  try {
   renderer = new THREE.WebGLRenderer({
    canvas,
    alpha: true,
    antialias: false, // points ronds adoucis par le shader → MSAA inutile, gros gain de fill-rate
  ...
  const N = initW < 520 ? 900 : initW < 900 ? 1200 : 1600;
  ...
  resize();
  // Redimensionnement piloté par ResizeObserver → plus de getBoundingClientRect
  // à chaque frame (évite un reflow forcé en boucle).
  let needsResize = false;
  const ro = ("ResizeObserver" in window) ? new ResizeObserver(() => { needsResize = true; }) : null;
  if (ro) ro.observe(canvas);
  const onWinResize = () => { needsResize = true; };
  window.addEventListener("resize", onWinResize, { passive: true });

  // Pause de l'animation quand le hero n'est plus à l'écran (gros gain FPS
  // pendant le scroll dans le catalogue).
  let inView = true;
  ...
  const loop = () => {
   ...
   // Hors écran : on garde la boucle vivante mais on ne calcule/rend rien.
   if (!inView) { clock.getDelta(); return; }
