...
# 2) Code applicatif. La structure backend/ + frontend/ est préservée : main.py
#    sert le frontend depuis ../frontend.
...
# Pour monter en charge, passer sur Postgres puis augmenter --workers.
# Le port est paramétrable via $PORT (Render/Heroku l'imposent) ; 8000 par défaut
# (Docker Compose local). Forme « shell » pour que ${PORT} soit bien substitué.
